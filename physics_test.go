...
func setup() *Engine {
    ...
    pComp := PComp{Ps: ps}
    pComp.Ps[grid] = make(map[Id]*V3)

    vs := make(map[*RefFrame]map[Id]*V3)
    vComp := VComp{Vs: vs}
    vComp.Vs[grid] = make(map[Id]*V3)

    // X,Y,Z coordinates in meters
    pComp.Ps[grid][e0] = &V3{1000.0, 1000.0, 1000.0} 
    pComp.Ps[grid][e1] = &V3{1000.0, 1000.0, 1000.0}
    pComp.Ps[grid][e2] = &V3{1000.0, 1000.0, 1000.0}

    // X,Y,Z,M velocity vector, M is meters per second (m/s)
    vComp.Vs[grid][e0] = &V3{1,0,0} 
    vComp.Vs[grid][e1] = &V3{1,1,0}
    vComp.Vs[grid][e2] = &V3{1,1,1}
